    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            position => {
                ...
                document.getElementById(`${inputId}-status`).textContent = 'تم مشاركة الموقع بنجاح';
            },
            error => {
                document.getElementById(`${inputId}-status`).textContent = 'فشل مشاركة الموقع، أدخل الرابط يدويًا';
                console.error('Geolocation error:', error);
            }
        );
    } else {
        document.getElementById(`${inputId}-status`).textContent = 'المتصفح لا يدعم مشاركة الموقع';
    }
}
